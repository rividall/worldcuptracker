...
**Bracket-template override (`bracket.BRACKET_ORDER`).** Winner-linking only reconstructs adjacency for rounds already *played*; for future rounds the API leaves teams null, so we order by match id — which assumes id order == bracket order. That holds for **QF, SF and FINAL** (their ids are in bracket order: QF `537383`/`537384` feed one semi, `537385`/`537386` the other). It is **false for the ROUND OF 16**, whose two middle pairs are numbered into the wrong quarters. Verified against the live QF team assignments (2026-07-04): QF `537383`←R16 `375,376`; `537384`←`379,380`; `537385`←`377,378`; `537386`←`381,382` — i.e. R16 `(379,380)` and `(377,378)` are transposed vs id order. So `BRACKET_ORDER` pins **only `LAST_16`**. Result: France & Spain in one half, Norway·England & Argentina in the other.

> **Correction (2026-07-04):** an earlier version also swapped the quarter-finals (`537384`↔`537385`). That was wrong — derived from reverse-engineering the R16→QF feeders *before* the QFs had real teams — and it crossed the halves (France drawn against the Norway/England winner in the semi). Once the QFs populated, the real feeders proved the QF ids never needed reordering. The QF override was removed; `LAST_16` remains.
