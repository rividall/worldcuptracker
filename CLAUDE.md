...
- **Timezones: store UTC, render in Spain time.** Match kickoff times are stored as timestamptz. The frontend renders them in `Europe/Madrid` for every viewer (not the viewer's local zone). The single source of truth for the display zone is `frontend/src/lib/time.ts` (`madridFormat`); the zone name (e.g. GMT+2) is shown so viewers abroad aren't misled into reading it as their own local time.
