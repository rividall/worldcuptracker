...
- [x] **[Bug] Bracket halves crossed (again)** FIXED (2026-07-04) -- Once the QFs populated with real teams, France was shown meeting the Norway/England winner in the semi. Root cause: the earlier fix wrongly swapped the quarter-finals (`537384`↔`537385`) in `bracket.BRACKET_ORDER`, based on reverse-engineering R16→QF feeders before the QFs had teams. Real QF assignments proved QF ids are already in bracket order; **only `LAST_16` needs pinning**. Removed the QF override. Verified by running `build_positions` on live data: SF 537387 = FRA/MAR + ESP/BEL, SF 537388 = NOR/ENG + ARG/SUI (France & Spain one half, Argentina the other). See football-data-analysis.md §5.
